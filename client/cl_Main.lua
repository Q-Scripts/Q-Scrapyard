...
Citizen.CreateThread(function()
    ...
    exports['qtarget']:AddBoxZone('Scrap', Config.Locations.Ped, 1.00, 1.50, {
        ...
        maxZ = 20.32,
            ...
            options = {
                {
                    event = 'q_scrapyard:giveinformation',
...
RegisterNetEvent('q_scrapyard:giveinformation')
AddEventHandler("q_scrapyard:giveinformation", function()
    ...
    if not timers and IsPedInAnyVehicle(PlayerPedId()) then
        ...
        for i = 0, 5 do
            ...
            Wait(100)
            lib.callback("q_scrapyard:deletevehicle")
        end

        Wait(1200)
        lib.callback("q_scrapyard:deletedb")
        lib.callback("q_scrapyard:deletevehicle")
